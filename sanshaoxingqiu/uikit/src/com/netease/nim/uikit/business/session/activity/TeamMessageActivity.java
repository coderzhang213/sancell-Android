...
import com.netease.nim.uikit.business.session.dialog.TeamNoticeDialog;
...
public class TeamMessageActivity extends BaseMessageActivity {
    ...
    protected void onResume() {
        ...
        //判断是否显示notice
        if (!isLoad && UserPreferences.isShowNotice(team.getId())){
            if (!TextUtils.isEmpty(team.getAnnouncement())){
                //判断是否是字典,ios公告内容是字典格式
                String str;
                try {
                    String jsonStr = "";
                    if (team.getAnnouncement().startsWith("[")){
                        jsonStr = team.getAnnouncement().substring(1,team.getAnnouncement().length() - 1);
                    }else{
                        jsonStr = team.getAnnouncement();
                    }
                ...
                TeamNoticeDialog dialog = new TeamNoticeDialog(this,str);
                dialog.show();
                UserPreferences.saveNotice(team.getId(),false);
