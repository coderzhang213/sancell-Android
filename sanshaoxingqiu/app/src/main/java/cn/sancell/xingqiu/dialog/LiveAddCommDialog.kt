...
class LiveAddCommDialog(contxt: Context) : Dialog(contxt, R.style.DaoxilaDialog_Alert), View.OnClickListener {
    var mOnUpRemmodityNameLinsener: OnUpComfirLinsener? = null
    var tv_title_alter: TextView? = null
    var et_new_name: EditText? = null
    override fun onClick(v: View?) {
        when (v?.id) {
            ...
            R.id.tv_comfirm -> {
                val newNmae = et_new_name?.text.toString()
                if (TextUtils.isEmpty(newNmae)) {
                    ToastHelper.showToast("请输入新的名称")
                    return
                }
                mOnUpRemmodityNameLinsener?.onComfirmLinser(newNmae)
                dismiss()
            }
        }
    }
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContentView(R.layout.view_live_add_comm_layout)
        //        //宽度全屏显示
