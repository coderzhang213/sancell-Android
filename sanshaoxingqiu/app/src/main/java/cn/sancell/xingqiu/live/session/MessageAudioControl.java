package cn.sancell.xingqiu.live.session;
...
public class MessageAudioControl extends BaseAudioControl<IMMessage> {
	...
	private TAdapter mAdapter = null;
			...
			long delayMillis)
	{
		if (StorageUtil.isExternalStorageExist()) {
			...
			Toast.makeText(mContext, com.netease.nim.uikit.R.string.sdcard_not_exist_error, Toast.LENGTH_SHORT).show();
		}	
	}
	
	private boolean playNextAudio(TAdapter tAdapter, IMMessage messageItem) {
	...
	public void setPlayNext(boolean isPlayNext, TAdapter adapter, IMMessage item) {
