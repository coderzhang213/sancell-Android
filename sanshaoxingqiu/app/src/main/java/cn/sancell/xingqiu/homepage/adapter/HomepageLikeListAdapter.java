...
public class HomepageLikeListAdapter extends BaseQuickAdapter<LikeBean, BaseViewHolder> {
    private int currentPage = 0;
    private UserBean userBean;

    public HomepageLikeListAdapter(int layoutResId, @Nullable List<LikeBean> data) {
        super(layoutResId, data);
        userBean = PreferencesUtils.readObject(Constants.Key.KEY_USERINFO, UserBean.class);
    }


    public HomepageLikeListAdapter(@Nullable List<LikeBean> data) {
        super(R.layout.item_homepage_like_list, data);
    }

    public HomepageLikeListAdapter(int layoutResId) {
        super(layoutResId);
    }
