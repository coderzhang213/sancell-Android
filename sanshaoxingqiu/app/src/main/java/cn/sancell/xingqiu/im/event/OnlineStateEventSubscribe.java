...
public class OnlineStateEventSubscribe {
    ...
    public static void initSubscribes() {
        ...
        if (timeInterval <= SUBS_FREQ) {
            ...
            Handler handler = Handlers.sharedHandler(ImCache.getContext());
    ...
    private static long getSubsTimeInterval() {
        if (lastSubsTime < 0) {
            lastSubsTime = NimSDKOptionConfig.KEY_SUBSCRIBE_TIME;
        }
    ...
    private static void updateLastSubsTime() {
        ...
        NimSDKOptionConfig.KEY_SUBSCRIBE_TIME = lastSubsTime;
    }
