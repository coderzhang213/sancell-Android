...
//            }
     ...
     * @param labels
     */
    private int mark_resoureid;

    public void setLabels(List<String> labels, int mark_resoureid) {
        this.mark_resoureid = mark_resoureid;
        setLabels(labels, new LabelTextProvider<String>() {
            @Override
            public CharSequence getLabelText(TextView label, int position, String data) {
                return data.trim();
     ...
     * @param <T>
     */
    public <T> void setLabels(List<T> labels, LabelTextProvider<T> provider, int mark_resoureid) {
        this.mark_resoureid = mark_resoureid;
        //清空原有的标签
        innerClearAllSelect();
        removeAllViews();
    ...
    private <T> void addLabel(T data, int position, LabelTextProvider<T> provider) {
        ...
        label.setMaxLines(1);
        //label.setMaxWidth(ScreenUtils.dip2px(mContext, 194));
        label.setEllipsize(TextUtils.TruncateAt.END);
        if (mark_resoureid != 0) {
            if (position == 0 || position == 1 || position == 2) {
                label.setCompoundDrawablesWithIntrinsicBounds(0, 0, mark_resoureid, 0);
                label.setCompoundDrawablePadding(6);
            } else {
