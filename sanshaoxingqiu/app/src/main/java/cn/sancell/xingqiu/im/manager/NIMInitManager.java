...
public class NIMInitManager {
    ...
    private void registerIMMessageFilter() {
        NIMClient.getService(MsgService.class).registerIMMessageFilter(message -> {
            if (NimSDKOptionConfig.KEY_MSG_IGNORE && message.getAttachment() != null) {
    ...
    private void registerBroadcastMessages(boolean register) {
        NIMClient.getService(MsgServiceObserve.class).observeBroadcastMessage(
                (Observer<BroadcastMessage>) broadcastMessage -> ToastHelper.showToast(ImCache.getContext(), "收到全员广播 ：" + broadcastMessage.getContent()), register);
    }
