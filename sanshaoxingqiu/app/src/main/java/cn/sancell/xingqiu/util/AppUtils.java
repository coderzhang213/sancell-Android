...
public class AppUtils {
    ...
    public static String getDeviceId(Context context) {
        ...
        //String md5 = RSAUtils.md5(s.toString());
        if (s.length() > 0) {
            //持久化操作, 进行保存到SD卡中
            saveDeviceID(s.toString(), context);
        }
        return s.toString();
    }
...
//        return info.getMacAddress();
    ...
    public static UserMemberRes getUserMember(){
      return PreferencesUtils.readObject(UiHelper.KEY_USER_MEMBER, UserMemberRes.class);
    }

    public static int getUserMemberCanUserRp(){

        UserMemberRes res = getUserMember();
        if (res != null){
            return res.useCouponPoint;
        }else{
            return  0;
        }
    }
