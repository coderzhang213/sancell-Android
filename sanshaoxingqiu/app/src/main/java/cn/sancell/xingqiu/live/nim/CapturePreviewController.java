...
import cn.sancell.xingqiu.live.network.NetworkMonitor;
import cn.sancell.xingqiu.live.network.NetworkUtils;
import cn.sancell.xingqiu.live.utils.VcloudFileUtils;
import cn.sancell.xingqiu.live.widget.MixAudioDialog;
import cn.sancell.xingqiu.live.widget.MixAudioLayout;
import cn.sancell.xingqiu.live.widget.NetWorkInfoDialog;
...
public class CapturePreviewController extends CapturePreviewContract.CapturePreviewControllerBase implements lsMessageHandler {
    ...
    public void handleIntent(Bundle intent) {
        mPublishParam = (PublishParam) intent.getSerializable(PublishParam.EXTRA_PARAMS);
        if (mPublishParam == null) {
            showToast("未传递参数,无法正常初始化");
            return;
        }
        switch (mPublishParam.definition) {//默认升一级
            case PublishParam.HD:
            case PublishParam.SD:
                mPublishParam.definition = PublishParam.HD;
                break;
            case PublishParam.LD:
                mPublishParam.definition = PublishParam.SD;
                break;
        }
     ...
     * @return
     */
    public boolean isCaptureCencar() {
    ...
    private void init() {
        ...
        if (mLSMediaCapture != null && captureWrapper.isVideo()) {
            mLSMediaCapture.startVideoPreview((NeteaseView)
                            ui.getDisplaySurfaceView(mUseFilter),
                    captureWrapper.getCameraPosition(),
                    mUseFilter, videoQuality, false);
        ...
        public void run() {
            ...
            if (mLSMediaCapture.getCameraZoomValue() < targetValue) {
                while (mLSMediaCapture.getCameraZoomValue() < targetValue) {
                    ...
                    if (latestProgress != mInitProgress)
                        return; //执行过程中,又有新的progress,故放弃执行当前Runnable
