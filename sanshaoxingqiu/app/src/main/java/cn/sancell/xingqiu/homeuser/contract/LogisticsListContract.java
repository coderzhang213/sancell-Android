...
public class LogisticsListContract {
    public interface LogisticsListView extends BaseView {
        ...
        void getLogisticsListSuccess(LogisticsListBean logisticsListBean);
        void getLogisticsInfoSuccess(LogisticsInfoBean logisticsInfoBean);
    }

    public static class LogisticsListPresenter extends BasePresenter<LogisticsListContract.LogisticsListView> {


        /**
         * 物流列表
         */
        public void GetLogisticsListList(String orderId, String parcelId, final Context context) {
            ...
            RetrofitUtil.getInstance().initRetrofit().getLogisticsListData(map)
                    .subscribeOn(Schedulers.io())
                    .observeOn(AndroidSchedulers.mainThread())
                    .subscribe(new BaseObserver<LogisticsListBean>(context) {
                        @Override
                        protected void onSuccess(BaseEntry<LogisticsListBean> t) throws Exception {
                            getView().showLoading(false);
                            if (t.getRetCode() == 0) {
                                if (t.getRetData() != null) {
                                    if (t.getRetData() != null) {
                                        getView().getLogisticsListSuccess(t.getRetData());
                                    }
                                }
                            } else {
