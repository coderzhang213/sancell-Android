package cn.sancell.xingqiu.live.http;
...
import cn.sancell.xingqiu.live.bean.AddVideoResponseEntity;
import cn.sancell.xingqiu.live.bean.RoomInfoEntity;
import cn.sancell.xingqiu.live.bean.TranscodeResponseEntity;
import cn.sancell.xingqiu.live.bean.VideoInfoEntity;
import cn.sancell.xingqiu.live.user.LiveCache;
import cn.sancell.xingqiu.live.utils.AndTools;
...
public class DemoServerHttpClient {
    ...
    private DemoServerHttpClient() {
        NimHttpClient.getInstance().init(LiveCache.getContext());
    }
            ...
            public void onResponse(String response, int code, Throwable exception) {
                ...
                try {
                    ...
                    if (resCode == RESULT_CODE_SUCCESS) {
                        ...
                        if (dataObject != null) {
                            LiveCache.setAccount(dataObject.getString(REQUEST_ACCID));
                            LiveCache.setToken(dataObject.getString(RESULT_KEY_IMTOKEN));
                            LiveCache.setVodtoken(dataObject.getString(RESULT_KEY_VODTOKEN));
                            LiveCache.setSid(dataObject.getString(REQUEST_ACCID));
                        }
                        callback.onSuccess(null);
                    } else {
                        String error = resObj.getString(RESULT_KEY_ERROR_MSG);
    ...
    public void logout(){
        ...
        StringBuilder body = new StringBuilder();
        body.append(REQUEST_SID).append("=").append(LiveCache.getSid());
    ...
    public void createRoom(Activity context, final DemoServerHttpCallback<RoomInfoEntity> callback){
        ...
        StringBuilder body = new StringBuilder();
        body.append(REQUEST_SID).append("=").append(LiveCache.getSid()).append("&")
    ...
    public void anchorLeave(String roomId, final DemoServerHttpCallback<Void> callback){
        ...
        StringBuilder body = new StringBuilder();
        body.append(REQUEST_SID).append("=").append(LiveCache.getSid()).append("&").append(REQUEST_ROOM_ID)
    ...
    public void addVideo(long vid, String name, int type, final DemoServerHttpCallback<AddVideoResponseEntity> callback){
        ...
        body.put(REQUEST_SID, LiveCache.getSid());
        body.put(VID, vid);
    ...
    public static String readAppKey() {
        try {
            ApplicationInfo appInfo = LiveCache.getContext().getPackageManager()
                    .getApplicationInfo(LiveCache.getContext().getPackageName(), PackageManager.GET_META_DATA);
            if (appInfo != null) {
