...
public abstract class RecyclerViewBannerBase<L extends RecyclerView.LayoutManager, A extends RecyclerView.Adapter> extends FrameLayout {
    ...
    protected Drawable indicatorNumberBackgroundDrawable;
    protected IndicatorAdapter indicatorAdapter;
    protected int indicatorMargin;//指示器间距

    protected boolean showNumberIndicator;//是否显示数字指示器
    protected TextView numberIndicatorContainer;
    ...
    protected void initView(Context context, AttributeSet attrs) {
        ...
        showIndicator = a.getBoolean(R.styleable.RecyclerViewBannerBase_showIndicator, false);
        showNumberIndicator = a.getBoolean(R.styleable.RecyclerViewBannerBase_showNumberIndicator, true);
        autoPlayDuration = a.getInt(R.styleable.RecyclerViewBannerBase_interval, 3000);
        ...
        indicatorNumberBackgroundDrawable = a.getDrawable(R.styleable.RecyclerViewBannerBase_indicatorNumberBackground);
        ...
        int marginRight = a.getDimensionPixelSize(R.styleable.RecyclerViewBannerBase_indicatorMarginRight, dp2px(12));
        int marginBottom = a.getDimensionPixelSize(R.styleable.RecyclerViewBannerBase_indicatorMarginBottom, dp2px(30));
        ...
        numberIndicatorContainer = new TextView(context);
        numberIndicatorContainer.setTextColor(Color.WHITE);
        numberIndicatorContainer.setTextSize(13);
        numberIndicatorContainer.setIncludeFontPadding(false);
        numberIndicatorContainer.setGravity(Gravity.CENTER);
        if (indicatorNumberBackgroundDrawable != null) {
            numberIndicatorContainer.setBackground(indicatorNumberBackgroundDrawable);
        }
        LayoutParams number_params = new LayoutParams(ViewGroup.LayoutParams.WRAP_CONTENT,
                ViewGroup.LayoutParams.WRAP_CONTENT);
        number_params.gravity = Gravity.BOTTOM | Gravity.RIGHT;
        number_params.setMargins(marginLeft, 0, marginRight, marginBottom);
        addView(numberIndicatorContainer, number_params);
        if (!showNumberIndicator) {
            numberIndicatorContainer.setVisibility(GONE);
        }
        if (!showIndicator) {
    ...
    public void setShowNumberIndicator(boolean showNumberIndicator) {
        ...
        numberIndicatorContainer.setVisibility(showNumberIndicator ? VISIBLE : GONE);
    }
    ...
    public void initBannerImageView(@NonNull List<String> newList, OnBannerItemClickListener onBannerItemClickListener) {
        ...
        if (compareListDifferent(newList, tempUrlList)) {
            ...
            if (bannerSize > 1) {
                ...
                numberIndicatorContainer.setVisibility(VISIBLE);
                numberIndicatorContainer.setText("1/" + bannerSize);
    ...
    protected synchronized void refreshIndicator() {
        ...
        if (showNumberIndicator && bannerSize > 1) {
            numberIndicatorContainer.setText(currentIndex % bannerSize+1+"/"+bannerSize);
        }
    }
